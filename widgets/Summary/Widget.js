  ...
  function(declare, BaseWidget, LayerInfos, utils,
    ...
    return declare([BaseWidget], {
      ...
      _clusterClick: function(evt) {
        ...
        // var title = this.count + " Features";
        // if (count === 1) {
        //   title = count + " Feature";
        // }
        var title = this.nls.features + ": " + count;
      ...
      _getFilterField: function(name) {
        var fld;
        array.some(this.targetLayer.fields, function(f) {
          if (f.name === name) {
            fld = f;
            return true;
          }
      ...
      _summarize: function() {
        ...
        this.sumTimer = setTimeout(lang.hitch(this, this._summarizeFeatures), 900);
      },
      ...
      _queryFeatures: function(ext) {
        ...
        this.defQuery = this.opLayer.queryIds(query, lang.hitch(this, function(objectIds) {
          this.summaryIds = objectIds;
          if (this.summaryIds.length > 0) {
            this._queryFeaturesByIds(true);
          } else {
            this.clusterLayer.setFeatures([]);
      ...
      _queryFeaturesByIds: function(isInitial) {
        if(isInitial){
          this.summaryFeatures = [];
        }
      ...
      _showMessage: function(msg) {
        ...
        this.messageTextNode.innerHTML = utils.sanitizeHTML(msg ? msg : '');
      ...
      _appyFilter: function(open) {
        if(this.filterExpr) {
          if (open) {
            this.opLayer.setDefinitionExpression(this.filterExpr);
            this.targetLayer.setDefinitionExpression(this.filterExpr);
          } else {
            this.opLayer.setDefinitionExpression("1=1");
            this.targetLayer.setDefinitionExpression("1=1");
          }
        }
      },
