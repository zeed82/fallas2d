...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
    ...
    _configuredDirectionUnit: null, // to store the actual configured direction unit
    _configuredDistanceUnit: null, // to store the actual configured distance unit
    ...
    constructor: function (options) {
      ...
      this._configuredDirectionUnit = null;
      this._configuredDistanceUnit = null;
      this._hasMultipleSourcesInSearch = true;
    ...
    _initializeCoordinatesWidget: function () {
      this._configuredDirectionUnit = lang.clone(this.config.traverseDirectionUnit);
      this._configuredDistanceUnit = lang.clone(this.config.traverseUnit);
      this._nodes = [];
      this._itemList = [];
    ...
    _attachLocateStartPointEvents: function () {
      ...
      this.own(on(this.locateStartPointIcon, "click", lang.hitch(this, function (evt) {
        Event.stop(evt);
        if (domClass.contains(this.locateStartPointIcon, "esriCTLocateIconSelected")) {
          this.deactivateLocateIcon();
        } else {
      ...
      this.own(on(this.locateLatAndLong, "click", lang.hitch(this, function (evt) {
        Event.stop(evt);
    ...
    _enableOrDisableTraverseGrid: function (gridEnabledFlag) {
      if (gridEnabledFlag) {
        ...
        domAttr.set(this.bearingNode, 'tabindex', "0");
        domAttr.set(this.lengthNode, 'tabindex', "0");
        domAttr.set(this.addButton, 'tabindex', "0");
      } else {
        ...
        domAttr.set(this.bearingNode, 'tabindex', "-1");
        domAttr.set(this.lengthNode, 'tabindex', "-1");
        domAttr.set(this.addButton, 'tabindex', "-1");
      }
    },
    ...
    _attachTraverseGridEvents: function () {
      this.own(on(this.addButton, "click", lang.hitch(this, function (evt) {
        Event.stop(evt);
        if (domClass.contains(this.addButton, "esriCTAddIcon")) {
          this._addNewItem();
        }
      })));
      this.own(on(this.addButton, 'keydown', lang.hitch(this, function (evt) {
      ...
      this.own(on(this.expandCollapseNode, 'click', lang.hitch(this, function (evt) {
        Event.stop(evt);
    ...
    _createFieldInputs: function (nodeContainer, value, className, isPopup) {
      ...
      if (className === "esriCTBearingRow") {
        ariaLabelStr = this.nls.traverseSettings.bearingLabel;
      } else if (className === "esriCTLengthRow") {
        ariaLabelStr = this.nls.traverseSettings.lengthLabel;
      }
    ...
    _createDeleteButton: function (row, index) {
      ...
      deleteIcon = domConstruct.create("div", {
        ...
        "tabindex": 0,
        "aria-label": this.nls.traverseSettings.deleteBearingAndLengthLabel,
        "role": "button"
      }, deleteButton);
      ...
      this.own(on(deleteIcon, "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          this.emit("restrictFocusOnFirstNode");
          var rowIndex = parseInt(domAttr.get(evt.currentTarget, "rowIndex"), 10);
          this._deleteRow(row, rowIndex);
    ...
    _showHideTraverseTools: function () {
      if (this._itemList && this._itemList.length > 0) {
        ...
        domAttr.set(this.expandCollapseNode, 'tabindex', "0");
      } else {
        domClass.add(this.expandCollapseNode, "esriCTHidden");
        domAttr.set(this.expandCollapseNode, 'tabindex', "-1");
      }
    },
    ...
    _onExpandCollapseClicked: function () {
      ...
      if (domClass.contains(this.expandCollapseNode, "esriCTExpand")) {
        ...
        domAttr.set(this.expandCollapseNode, "aria-label",
          this.nls.planSettings.collapseGridTooltipText);
      ...
      this._setFocusOnExpandCollapseGridButton();
    },
    ...
    _handleEventsOnInitialRow: function () {
      ...
      this.own(on(this.bearingNode, 'keydown', lang.hitch(this, function (evt) {
        this._bearingValueEntered(evt);
        if (evt.keyCode === keys.ESCAPE) {
          if (!this.isSingleTabSelected) {
            Event.stop(evt);
            this.emit("focusLastSelectedTab");
      ...
      this.own(on(this.lengthNode, 'keydown', lang.hitch(this, function (evt) {
        this._lengthValueEntered(evt);
        if (evt.keyCode === keys.ESCAPE) {
          if (!this.isSingleTabSelected) {
            Event.stop(evt);
            this.emit("focusLastSelectedTab");
    ...
    _attachPlanSettingsIconClickEvent: function () {
      this.own(on(this.planSettingIcon, 'click', lang.hitch(this, function (evt) {
        Event.stop(evt);
        this._openPlanSettingsDialogBox(evt);
      })));
    },
    ...
    _openPlanSettingsDialogBox: function (evt) {
      ...
      this.own(on(this.planSettingsWidget, 'setFocusOnPlanSettingsIcon', lang.hitch(this, function () {
        this._focusOutCurrentNode();
        focusUtil.focus(this.planSettingIcon);
      })));
    },
    ...
    _attachLatLongTextBoxEvents: function () {
      this.own(on(this.latitudeTextBox, "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          this.latitudeTextBox.setValue(utils.honourPopupRounding(8, this.latitudeTextBox.getValue()));
          if (this.latitudeTextBox.isValid() && this.longitudeTextBox.isValid()) {
            this._onLocateLatLongClicked();
            setTimeout(lang.hitch(this, function () {
      ...
      this.own(on(this.longitudeTextBox, "keydown", lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
    ...
    resetCoordinatesWidgetValue: function () {
      ...
      if (this.planSettingsWidget && this.planSettingsWidget.config) {
        this.planSettingsWidget.config.traverseDirectionUnit = this._configuredDirectionUnit;
        this.planSettingsWidget.config.traverseUnit = this._configuredDistanceUnit;
        this.planSettingsWidget.emit("planSettingsChanged",
          this._configuredDirectionUnit, this._configuredDistanceUnit);
      }
    },
    ...
    focusFirstNodeOfSelectedTab: function () {
      focusUtil.focus(this.coordinatesHeader);
    },
    ...
    setFirstFocusNode: function (domNodeObj) {
      jimuUtils.initFirstFocusNode(domNodeObj, this.coordinatesHeader);
    },

    /**
     * This function is used to attach key events to the element
     */
    _attachKeydownEventToTheNodes: function () {
      this.own(on(this.locateStartPointIcon, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          if (domClass.contains(this.locateStartPointIcon, "esriCTLocateIconSelected")) {
            this.deactivateLocateIcon();
      ...
      this.own(on(this.locateLatAndLong, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          if ((!this.latitudeTextBox.isValid()) || (!this.longitudeTextBox.isValid())) {
            this._showMessage(this.nls.coordinatesWidget.invalidLatLongMessage);
            ...
            setTimeout(lang.hitch(this, function () {
              this._focusOutCurrentNode();
              focusUtil.focus(this.startPointSection);
            }), 100);
          }
        }
        if (evt.keyCode === keys.ESCAPE) {
      ...
      this.own(on(this.planSettingIcon, 'keydown', lang.hitch(this, function (evt) {
        if (evt.keyCode === keys.ENTER || evt.keyCode === keys.SPACE) {
          Event.stop(evt);
          var position = domGeom.position(this.planSettingIcon);
          var eventObj = {};
          eventObj.pageX = position.x + 10;
          eventObj.pageY = position.y + 12;
          this._openPlanSettingsDialogBox(eventObj);
          this.planSettingsWidget.setFocusToPlanSettingsDialog();
        }
        if (evt.keyCode === keys.ESCAPE) {
          if (!this.isSingleTabSelected) {
            Event.stop(evt);
    ...
    /**
     * This function is used to set focus on expand and collapse grid button
     */
    _setFocusOnExpandCollapseGridButton: function () {
      this._focusOutCurrentNode();
      focusUtil.focus(this.expandCollapseNode);
    },

    /**
     * This function is used to attach keydown event to coordinates search input and submit node
     */
