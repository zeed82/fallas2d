...
define([
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    _selectLayerTable: null, // to store the object of select layer table
    ...
    _attachComponentEvents: function () {
      this.own(on(this.drawToolCheckBox, "change", lang.hitch(this, function (evt) {
        if (evt) {
          domClass.remove(this.selectDrawToolFieldSet, "esriCTHidden");
        } else {
          domClass.add(this.selectDrawToolFieldSet, "esriCTHidden");
        }
      })));
    },
    ...
    getConfig: function () {
      ...
      if (this._validateConfigForCheckBoxes() &&
        ...
        this.config.lengthUnits = this.lengthUnitsDropDown.get("value");
        ...
        this.config.reportSettings = {
          ...
          noDataConfiguredText: this._downloadTab.getNoDataText(),
          naTextConfiguredValue: this._downloadTab.getNaTextValue(),
          customOptions: this._downloadTab.getCustomOptions(),
          defaultPrintOption: this._downloadTab.getDefaultPrintOption(),
          displaySummaryInReport: this._downloadTab.getDisplaySummaryCheckBoxStatus(),
          hideZeroValueRow: this._downloadTab.getRowshavingAllZeroValueCheckBoxStatus(),
          hideNullValueRow: this._downloadTab.getRowshavingAllNullValueCheckBoxStatus()
        };
        this.config.drawToolSelectableLayers = this._getDrawToolSelectableLayers();
        if (this.pointSelectToolCheckBox) {
          this.config.showPointTool = this.pointSelectToolCheckBox.checked;
        }
        if (this.polylineSelectToolCheckBox) {
        ...
        if (this.circleSelectToolCheckBox) {
          this.config.showCircleTool = this.circleSelectToolCheckBox.checked;
        }
        return this.config;
      } else {
        return false;
    ...
    /**
     * This function is used to get the number of selected layers which should
     * be available in the draw tool panel
     */
    _getDrawToolSelectableLayers: function () {
      var selectedLayerIDArr, selectableLayerRows;
      selectedLayerIDArr = [];
      selectableLayerRows = this._selectLayerTable.getRows();
      array.forEach(selectableLayerRows, lang.hitch(this, function (row) {
        var rowData;
        rowData = this._selectLayerTable.getRowData(row);
        if (rowData.selectable) {
          selectedLayerIDArr.push(rowData.selectableLayerID);
        }
      }));
      return selectedLayerIDArr;
    },
    ...
    _validateLayers: function () {
      var isValid = true,
        layersArray = this._getSelectedLayers(false);
    ...
    _initConfiguration: function () {
      var symbology;
      this._symbolParams = {};
      this.defaultBufferDistanceTextBox.constraints = {
        min: 0
      };
      ...
      this.maxFeatureCountTextBox.constraints = {
        min: 1,
        places: 0
      };
      //Allow user's to see the conversion changes on each key press
      this.searchToleranceTextBox.constraints = {
        min: 0
      };
      ...
      on(this.lengthUnitsDropDown, "change", lang.hitch(this, function (value) {
    ...
    _initTabs: function () {
      ...
      this.own(on(this._configurationTabsContainer.containerNode, "scroll",
        lang.hitch(this, function () {
          ...
          this.lengthUnitsDropDown.closeDropDown();
      ...
      if (this.config) {
        ...
        if (this.config.areaUnits) {
          if (this.config.areaUnits === "Standard" ||
            ...
            this.config.areaUnits === "Meters" ||
            this.config.areaUnits === "Kilometers" ||
            this.config.areaUnits === "Hectares") {
            // backward compatibility for area dropdown
            this._setBackwardCompatibilityAreaUnit(this.areaUnitsDropDown, this.config.areaUnits);
            // backward compatibility for length dropdown
            if (!this.config.hasOwnProperty('lengthUnits')) {
              this._setBackwardCompatibilityLengthUnit(this.lengthUnitsDropDown, this.config.areaUnits);
            }
          } else {
            this._setDropDownSavedOptionSelected(this.areaUnitsDropDown, this.config.areaUnits);
          }
        }
        // set option for length dropdown
        if (this.config.lengthUnits) {
          this._setDropDownSavedOptionSelected(this.lengthUnitsDropDown, this.config.lengthUnits);
        }
        ...
        if (!this.config.hasOwnProperty('lengthUnits')) {
          ...
          this._displaySearchTolerance(this.lengthUnitsDropDown.get('value'));
        }
        ...
        if (this.drawToolCheckBox.checked) {
          domClass.remove(this.selectDrawToolFieldSet, "esriCTHidden");
        } else {
          domClass.add(this.selectDrawToolFieldSet, "esriCTHidden");
        }
        ...
        if (this.config.hasOwnProperty("showPointTool")) {
          this.pointSelectToolCheckBox.setValue(this.config.showPointTool);
        }
        if (this.config.hasOwnProperty("showPolylineTool")) {
        ...
        if (this.config.hasOwnProperty("showCircleTool")) {
          this.circleSelectToolCheckBox.setValue(this.config.showCircleTool);
        } else {
          // This code is not related to 508/accessibility. It added to solve issue related to circle tool.
          this.circleSelectToolCheckBox.setValue("false");
        }
      }
    },
    ...
    _onEditLayerClicked: function (tr) {
      var layerGridInfo = tr.layerInfo,
        args;
      this._curRow = tr;
      if (tr.layerSelector) {
        tr.layerSelector.hideLayerChooser();
    ...
    _createFieldSelectorPopup: function (fieldPickerArgs, tr) {
      var popup, okButton, cancelButton, isValid = true,
        lastSelectedFields;
    ...
    _setBackwardCompatibilityAreaUnit: function (dropDownNode, configValue) {
      switch (configValue) {
        ...
        case "Standard":
          configValue = "Acres";
          dropDownNode.set("value", "Acres");
          break;
        // level 1 -> backward compatibility
        case "Metric":
          configValue = "SquareKilometers";
          dropDownNode.set("value", "SquareKilometers");
          break;
        // level 2 -> backward compatibility
        case "Feet":
          configValue = "SquareFeet";
          dropDownNode.set("value", "SquareFeet");
          break;
        // level 2 -> backward compatibility
        case "Miles":
          configValue = "Acres";
        ...
        // level 2 -> backward compatibility
        case "Meters":
          configValue = "SquareMeters";
          dropDownNode.set("value", "SquareMeters");
          break;
        // level 2 -> backward compatibility
        case "Kilometers":
          configValue = "SquareKilometers";
        ...
        // level 2 -> backward compatibility
        case "Hectares":
          configValue = "Hectares";
          dropDownNode.set("value", "Hectares");
          break;
      }
    ...
    /**
     * This function is used to set the length unit dropdown
     * for backward value like standard & metric.
     * @memberOf Screening/setting/Setting
     */
    _setBackwardCompatibilityLengthUnit: function (dropDownNode, configValue) {
      switch (configValue) {
        // level 1 -> backward compatibility
        case "Standard":
          configValue = "Miles";
      ...
      array.forEach(dropDownNode.options, lang.hitch(this,
    ...
    _createPreviewContainer: function (symbolNode, showPreviewText) {
      ...
      if (showPreviewText) {
        ...
        trPreviewText = domConstruct.create("tr", {
          "class": "esriCTTRPreviewText"
        },
          tablePreviewText);
    ...
    _setDownloadInfo: function (tr) {
      var selectedLayerInfo, requestHandle;
      ...
      if (this._downloadTab) {
        ...
        if (selectedLayerInfo) {
          requestHandle = esriRequest({
            "url": selectedLayerInfo.url + "?f=pjson"
          });
          requestHandle.then(lang.hitch(this, function (response) {
            selectedLayerInfo.capabilities = response.capabilities;
    ...
    _getSelectedLayers: function (getCapabilities) {
      var layerItem, selectedLayersArr = [],
        trs;
      trs = this._addLayerTable.getRows();
    ...
    _getLayerInfo: function (tr, getCapabilities) {
      ...
      if (selectedLayerItem) {
        layerItem = {
          ...
          "label": jimuUtils.stripHTML(tr.labelText &&
    ...
    /**
     * This function is used to create a table used for selecting the layers.
     * Layers selected in this table will be available in the draw tool dropdown
     * @memberOf Screening/setting/Setting
     */
    _createSelectLayerGrid: function () {
      var args, fields;
      fields = [{
        "name": 'selectable',
        "title": "",
      ...
      this._selectLayerTable = new SimpleTable(args);
      this._selectLayerTable.placeAt(this.drawToolSelectableLayerContainer);
      this._selectLayerTable.startup();
    },

    /**
     * This function is used to populate selectable layer table
     * @memberOf Screening/setting/Setting
     */
    _populateSelectableLayer: function () {
      array.forEach(this._totalLayers, lang.hitch(this, function (layerObject) {
        var checkBoxCheckState;
        // for backward compatibility this needs to be set true.
        // because in this mode, select tool is always visible,
        // and so all the layers needs to be displayed.
        checkBoxCheckState = true;
        if (this.config && this.config.hasOwnProperty("drawToolSelectableLayers")) {
          checkBoxCheckState = false;
          if (this.config.drawToolSelectableLayers.indexOf(layerObject.id) > -1) {
            checkBoxCheckState = true;
          }
        }
        this._selectLayerTable.addRow({
          selectable: checkBoxCheckState,
          selectableLayerName: layerObject.title,
          selectableLayerID: layerObject.id
        });
      }));
    },
    ...
    _populateLayerTableRow: function (layerInfo, addFromButtonClick) {
      ...
      if (result.success && result.tr) {
        ...
        tr.groupbyfieldCheckBoxStatus = layerInfo.groupbyfieldCheckBoxStatus;
      ...
      if (this._idsOfSelectedLayers.length <
        ...
        unUsedLayers = array.filter(this._totalLayers,
          lang.hitch(this, function (layer) {
            return this._idsOfSelectedLayers.indexOf(layer.id) < 0;
          }));
      } else {
    ...
    _addLayerDropDown: function (tr, selectedLayer, addFromButtonClick) {
      var td, layerChooserFromMapArgs, layerSelector, layerChooserFromMap, j;
      if (tr.layerSelector) {
      ...
      if (selectedLayer) {
        ...
        // delete the row if layer is invalid or not added on map
        if (layerChooserFromMapArgs.createMapResponse.errors &&
          layerChooserFromMapArgs.createMapResponse.errors[0] &&
          layerChooserFromMapArgs.createMapResponse.errors[0].name &&
          layerChooserFromMapArgs.createMapResponse.errors[0].name === "Error" &&
          layerSelector.getSelectedItem() === null) {
          for (j = 0; j < this.config.downloadSettings.layers.length; j++) {
            if (selectedLayer.url === this.config.downloadSettings.layers[j].url) {
              this.config.downloadSettings.layers.splice(j, 1);
              break;
            }
          }
    ...
    _validateDrawTools: function () {
      if (this.drawToolCheckBox.checked) {
        if ((!this.pointSelectToolCheckBox.checked) &&
          (!this.polylineSelectToolCheckBox.checked) &&
          (!this.extentSelectToolCheckBox.checked) &&
          (!this.polygonSelectToolCheckBox.checked) &&
          (!this.circleSelectToolCheckBox.checked) &&
          (this._getDrawToolSelectableLayers().length === 0)) {
          this._errorMessage(this.nls.analysisTab.selectingDrawToolErrorMessage, this.nls.analysisTab.analysisTabLabel);
          return false;
        }
      }
      return true;
